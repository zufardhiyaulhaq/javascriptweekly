...
import (
	...
	"github.com/zufardhiyaulhaq/javascriptweekly/pkg/scrappers"
)

type JavascriptWeekly struct {
	URL string
}

func NewJavascriptWeekly(URL string) scrappers.Scrapper {
	return JavascriptWeekly{
		URL: URL,
	}
}

func (g JavascriptWeekly) GetName() (string, error) {
	...
	name := "Javascript Weekly " + regex.FindString(text)

	return name, nil
}

func (g JavascriptWeekly) GetArticles() ([]communityv1alpha1.ArticleSpec, error) {
	var articles []communityv1alpha1.ArticleSpec

	response, err := http.Get("https://javascriptweekly.com/issues/latest?layout=bare")
	if err != nil {
		return articles, err
	}
